# git-credential-helper Specification

## Purpose
Supply the HTTPS push credential to git at invocation time from a single
`VAULT_GIT_TOKEN` environment variable, via an env-reading git credential helper,
so the token is never written to `.git/config` nor passed on a git argument
vector and rotation is a single-variable change.
